...
async function main(): Promise<any> {
  try {
    ...
    const macd: any = MACD.calculate(macdInput)

    // console.log("macd", macd)
    console.log("macd", macd[macd.length - 1])
    const { MACD: currentMacd, signal: currentSignal } = macd[macd.length - 1]
    console.log(`== Current MACD: ${currentMacd}`)
    ...
    if (currentMacd === currentSignal) {
      return "==> MACD and Signal are equal - position is NEUTRAL, no action taken"
    }
